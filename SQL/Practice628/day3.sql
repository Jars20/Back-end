drop database if exists day3_homework;
...
alter table tb_dept add constraint emp_dept_fk foreign key (deptId) references tb_dept (id);
...
(
    ...
    deptId INT(11),
    salary FLOAT,
    constraint emp_dept_fk foreign key (deptId) references tb_dept (id)
);

-- 建立名为tb_dept1的表，结构和tb_dept表结构一致，将tb_emp外键删除，关联到tb_dept1上
create table tb_dept1 like tb_dept;
insert into tb_dept1 select * from tb_dept;
...
!!!select	f_name from suppliers s 
inner join fruits f on f.s_id =s.s_id
where f.f_id = 'a1';
...
select	f.f_id, f.s_id, f.f_name, f.f_price, s.s_name from fruits f
...
inner join student stu on s.stu_id = stu.id
where stu.name = '李四';
...
-- 计算每个学生的总成绩
select stu.id,stu.name,sum(s.grade) from score s
inner join student stu on s.stu_id = stu.id
group by stu.id;

-- 计算每个考试科目的平均成绩
select s.c_name,avg(s.grade) from score s
inner join student stu on s.stu_id = stu.id
group by s.c_name;
...
！！！select stu.id,stu.sex,stu.birth,stu.department from score s
inner join student stu on s.stu_id = stu.id
group by stu.id
having s.c_name='计算机' and s.c_name='英语';

-- 从student表和score表中查询出学生的学号，然后合并查询结果
select * from score s
right join student stu on s.stu_id = stu.id;

-- 查询姓张或者姓王的同学的姓名、院系和考试科目及成绩
select stu.name,stu.department,s.grade from score s
inner join student stu on s.stu_id = stu.id
where stu.name like '张%' or stu.name like '王%';

-- 查询都是湖南的学生的姓名、年龄、院系和考试科目及成绩
select stu.name,(2020-stu.birth) as age,stu.department,s.c_name,s.grade from score s
inner join student stu on s.stu_id = stu.id
...
-- 取得每个部门最高薪水的人员名称
select maxsal.e deptno,emp.ename 
from emp,(select emp.deptno e,max(sal) a from emp 
inner join dept on emp.deptno=dept.deptno group by emp.deptno) maxsal 
where maxsal.e=emp.deptno and maxsal.a=emp.sal;

-- 哪些人的薪水在部门的平均薪水之上
select emp.ename 
from emp,(select emp.deptno e,avg(sal) av from emp 
inner join dept on emp.deptno=dept.deptno group by emp.deptno) avgsal 
where avgsal.e = emp.deptno and emp.sal>avgsal.av;

-- 取得部门中(所有人)平均薪水的等级

-- 取得部门中(所有人)薪水的平均等级
select emp.deptno department,avg(sal) av from emp 
...
-- 列出至少有五个员工的部门详细信息
select d.* from (select deptno,count(*) as count from emp group by deptno) temp 
inner join dept d on temp.deptno = d.deptno
where temp.count>=5;

-- 列出所有“CLERK”(办事员)的姓名及其部门名称，部门人数
select e.ename,d.dname,counts.num from emp e
inner join (select e.deptno,count(*) num from emp e group by e.deptno) counts on counts.deptno=e.deptno
