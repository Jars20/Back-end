...
-- 查询价格为10.2元的水果的名称
select f_name as 名称 from fruits where f_price=10.2;

-- 查找名称为“apple”的水果的价格
select f_price as 名称 from fruits where f_name='apple';

-- 查询价格小于10的水果的名称
select f_name as 名称 from fruits where f_price<10;
...
select f_name as 名称 from fruits where f_name like 'b%y';
...
select dept_no as 部门,count(e_no) as 人数 from employee group by dept_no;
...
select c_id,c_name,c_email from customers where not c_email  is null;
...
select f_name,f_price from fruits where f_name='apple' and f_price>5;
...
select s_id,count(f_name) from fruits group by s_id,f_name;
...
select o_num,(quantity*item_price) as totalprice from orderitems where (quantity*item_price)>100 order by totalprice;
...
select count(*) from customers where not c_email is null;

-- 在orderitems表中，使用COUNT()函数统计不同订单号中订购的水果种类
select count(o_item),o_num from orderitems group by o_num;

-- 在orderitems表中查询30005号订单一共购买的水果总量
select count(o_item),o_num from orderitems group by o_num having o_num=30005;

-- 在orderitems表中，使用SUM()函数统计不同订单号中订购的水果总量
select sum(quantity),o_num from orderitems group by o_num;

-- 在fruits表中，查询s_id=103的供应商的水果价格的平均值
select avg(f_price) from fruits where s_id=103;

-- 在fruits表中，查询每一个供应商的水果价格的平均值
select avg(f_price),s_id from fruits group by s_id;
